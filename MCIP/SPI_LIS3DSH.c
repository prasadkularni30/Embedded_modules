...
/* USER CODE BEGIN PV */
uint8_t TxBuff[2]; 	// buffer to store the data to be transmitted
uint8_t Rx_x;
uint8_t Rx_y;
uint8_t Rx_z;
/* USER CODE END PV */
...
//configure Control Register 4 with address 0x20 and data 37h
  ...
  TxBuff[0]= 0x20;	// address of register
  TxBuff[1]= 0x17;	// Data

  HAL_SPI_Transmit(&hspi1, TxBuff, 2, 50);	//Data Transmitted
  ...
  while (1)
  {
	  //Ready the OUT_X(28h - 29h)
	  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, RESET);	//cs pulled low
	  TxBuff[0]= 0x29 | 0x80;
// To Read from OUT_X address we have make MSB to 1 so always do OR'|' Operation with 0x80
	  HAL_SPI_Transmit(&hspi1, TxBuff, 1, 50);
	  HAL_SPI_Receive(&hspi1, &Rx_x, 1, 50);

	  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, SET); //cs pulled high

	  //Ready the OUT_X(2Ah - 2Bh)
