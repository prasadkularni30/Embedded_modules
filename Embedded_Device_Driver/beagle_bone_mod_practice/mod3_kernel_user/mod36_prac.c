...
//static struct common temp;

static struct common *krnl;

// function prototypes
static int __init my_mod_init(void);
static int my_open(struct inode *inode, struct file *file);
static ssize_t my_read(struct file *file, char __user *vd, size_t len, loff_t *off);
static ssize_t my_write(struct file *file,const char __user *vd, size_t len, loff_t *off);
...
static ssize_t my_read(struct file *file, char __user *vd, size_t len, loff_t *off)
{
	...
	if ( copy_to_user(vd,(void *)krnl, sizeof(struct common)) )
	{
		pr_err("Error in data write to userspace!\n");
	}
	pr_info("Data written to userspace\n");
	return sizeof(struct common);
}
...
static ssize_t my_write(struct file *file, const char __user *vd, size_t len, loff_t *off)
{
	...
	if ( copy_from_user((void *)krnl, vd, len) )
	{
...
static int __init my_mod_init(void)
{
	...
	krnl = (struct common *)kmalloc(sizeof(struct common), GFP_KERNEL);
	if (!krnl)
	{
