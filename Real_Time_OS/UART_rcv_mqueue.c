...
void recvTask( void* NotUsed )
{
	...
	while(1)
	{
		SEGGER_SYSVIEW_PrintfHost("waiting for message");
		if(xQueueReceive(ledCmdQueue, &nextCmd, portMAX_DELAY) == pdTRUE)
		{
			SEGGER_SYSVIEW_PrintfHost("received message %d",nextCmd);
			switch(nextCmd)
			{
				case ALL_OFF:
					RedLed.Off();
...
void sendingTask( void* NotUsed )
{
	...
	while(1)
	{
		SEGGER_SYSVIEW_PrintfHost("checking UART");
		if( ( HAL_UART_Receive(&huart4,(uint8_t *) &ledCmd,sizeof(ledCmd), 10) ) == HAL_OK)
		{
			SEGGER_SYSVIEW_PrintfHost("command received %c",ledCmd);
			txmsg =atoi(&ledCmd);
			SEGGER_SYSVIEW_PrintfHost("transmitting %d",txmsg);
			xQueueSend(ledCmdQueue, &txmsg, portMAX_DELAY);
			vTaskDelay(200/portTICK_PERIOD_MS);
			SEGGER_SYSVIEW_PrintfHost("command transmitted");
		}
