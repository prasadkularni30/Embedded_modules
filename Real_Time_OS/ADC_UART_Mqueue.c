...
void recvTask( void* NotUsed )
{
	...
	while(1)
	{
		...
		if(xQueueReceive(ledCmdQueue, &nextCmd, portMAX_DELAY) == pdTRUE)
		{
			...
			if( (HAL_UART_Transmit(&huart4, (uint8_t *)&nextCmd, sizeof(nextCmd), 10) )==HAL_OK )
				SEGGER_SYSVIEW_PrintfHost("message transmitted successfully to UART");
			else
